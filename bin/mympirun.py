...
def get_mpi_and_sched_and_options():
    ...
    sched, found_sched = schedm.what_sched(getattr(optionparser.options, 'sched', None))
    ...
    if sched is None:
        optionparser.log.raiseException("No sched class found (options.sched %s ; found Sched classes %s)",
                                        optionparser.options.sched, ", ".join(found_sched_names))
    else:
        optionparser.log.debug("Found sched class %s from options.sched %s (all Sched found %s)",
                               sched.__name__, optionparser.options.sched, ", ".join(found_sched_names))
